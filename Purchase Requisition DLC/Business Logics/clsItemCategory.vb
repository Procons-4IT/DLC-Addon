Public Class clsItemCategory
    ...
    Private Sub LoadForm()
        oForm = oApplication.Utilities.LoadForm(xml_ItemCat, frm_ItemCat)
...
#Region "DataBind"
    Public Sub BindData(ByVal objform As SAPbouiCOM.Form)
        ...
        Try
            ...
            otestrs.DoQuery("Update ""@Z_ITCAT"" set Name=' '  where ""Name"" Like '%_XD'")
            oMatrix = objform.Items.Item("3").Specific
            oDBDataSrc = objform.DataSources.DBDataSources.Add("@Z_ITCAT")
            Try
                oDBDataSrc.Query()
...
#Region "Enable Matrix After Update"
    ...
    Private Sub EnblMatrixAfterUpdate(ByVal objApplication As SAPbouiCOM.Application, ByVal ocompany As SAPbobsCOM.Company, ByVal oForm As SAPbouiCOM.Form)
        ...
        Try
            ...
            End If
            If 1 = 1 Then
                oUserTable = ocompany.UserTables.Item("Z_ITCAT")
                oDBDSource = oForm.DataSources.DBDataSources.Item("@Z_ITCAT")
                ' oMatrix.DeleteRow(oMatrix.RowCount)
                oMatrix.FlushToDataSource()
                For i = 0 To oDBDSource.Size - 1
                    oGeneralService = oCompanyService.GetGeneralService("Z_ITCAT")
                    oGeneralData = oGeneralService.GetDataInterface(SAPbobsCOM.GeneralServiceDataInterfaces.gsGeneralData)
                    strCode = oDBDSource.GetValue("Code", i).Trim
                    otestRs = oApplication.Company.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)
                    oGeneralParams = oGeneralService.GetDataInterface(SAPbobsCOM.GeneralServiceDataInterfaces.gsGeneralDataParams)
                    otestRs.DoQuery("SElect * from [@Z_ITCAT] where Code='" & strCode & "'")
                    If otestRs.RecordCount > 0 Then
                        oGeneralParams.SetProperty("Code", strCode)
                        oGeneralData = oGeneralService.GetByParams(oGeneralParams)
                        oGeneralData.SetProperty("U_CatCode", oDBDSource.GetValue("U_CatCode", i))
                        oGeneralData.SetProperty("U_CatDesc", oDBDSource.GetValue("U_CatDesc", i))
...
#Region "Insert Code and Doc Entry"
    ...
    Public Sub InsertCodeAndDocEntry(ByVal aForm As SAPbouiCOM.Form)
        ...
        Try
            objForm = aForm
            aForm.Freeze(True)
            oDBDSource = objForm.DataSources.DBDataSources.Item("@Z_ITCAT")
            objMatrix = objForm.Items.Item("3").Specific
            objMatrix.FlushToDataSource()

            Dim strCode, strDocEntry As String
            strCode = oApplication.Utilities.getMaxCode("@Z_ITCAT", "Code")
            strDocEntry = oApplication.Utilities.getMaxCode("@Z_ITCAT", "DocEntry")
...
#End Region
        ...
        Try
            ...
            For intRow As Integer = aRow To aRow
                If oMatrix.IsRowSelected(intRow) Then
                    ...
                    oGeneralService = oCompanyService.GetGeneralService("Z_ITCAT")
                    oGeneralParams = oGeneralService.GetDataInterface(SAPbobsCOM.GeneralServiceDataInterfaces.gsGeneralDataParams)
                    otestRs.DoQuery("Update ""@Z_ITCAT"" set ""Name""=isnull(""Name"",'') + '_XD' where ""Code""='" & strCode & "'")
    ...
    Private Function Validation(ByVal aForm As SAPbouiCOM.Form) As Boolean
        ...
        For intRow As Integer = 1 To oMatrix.RowCount
            ...
            If strcode <> "" And strcode1 = "" Then
                oApplication.Utilities.Message("ItemCategory Description can not be empty", SAPbouiCOM.BoStatusBarMessageType.smt_Error)
                oMatrix.Columns.Item("V_1").Cells.Item(oMatrix.RowCount).Click(SAPbouiCOM.BoCellClickType.ct_Regular)
                Return False
            End If

            If strcode = "" And strcode1 <> "" Then
                oApplication.Utilities.Message("ItemCategory Code can not be empty", SAPbouiCOM.BoStatusBarMessageType.smt_Error)
    ...
    Private Function ValidateCode(ByVal strValue As String) As Boolean
        Try
            Dim strQuery As String
            Dim oRec As SAPbobsCOM.Recordset
            oRec = oApplication.Company.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)
            strQuery = "Select * from [@Z_ITCAT1] where U_CatCode='" & strValue & "'"
            oRec.DoQuery(strQuery)
            If oRec.RecordCount > 0 Then
                oApplication.Utilities.Message("This entry already mapped with login setup", SAPbouiCOM.BoStatusBarMessageType.smt_Error)
                Return False
            End If
            strQuery = "Select * from [OITM] where U_CatCode='" & strValue & "'"
            oRec.DoQuery(strQuery)
            If oRec.RecordCount > 0 Then
                oApplication.Utilities.Message("This entry already mapped with Item master data", SAPbouiCOM.BoStatusBarMessageType.smt_Error)
                Return False
            End If
            Return True
        Catch ex As Exception
...
#Region "Item Event"
    Public Overrides Sub ItemEvent(ByVal FormUID As String, ByRef pVal As SAPbouiCOM.ItemEvent, ByRef BubbleEvent As Boolean)
        Try
            If pVal.FormTypeEx = frm_ItemCat Then
                Select Case pVal.BeforeAction
                    Case True
                        If pVal.EventType = SAPbouiCOM.BoEventTypes.et_KEY_DOWN And pVal.CharPressed <> "9" And pVal.ItemUID = "3" And pVal.ColUID = "V_0" Then
                            ...
                            If ValidateCode(strVal) = False Then
                                BubbleEvent = False
                                Exit Sub
                            End If
                        End If
                    ...
                    Case False
                        If ((pVal.EventType = SAPbouiCOM.BoEventTypes.et_ITEM_PRESSED) And (pVal.ItemUID = "1")) Then
                            ...
                            Dim otestrs As SAPbobsCOM.Recordset
                            otestrs = oApplication.Company.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)
                            otestrs.DoQuery("Delete from ""@Z_ITCAT"" where ""Name"" Like '%_XD'")
                    ...
                    If pVal.BeforeAction = True Then
                        ...
                        For intRow As Integer = 1 To objMatrix.RowCount
                            If objMatrix.IsRowSelected(intRow) Then
                                ...
                                'If oApplication.Utilities.ValidateCode(strValue, "LANG") = True Then
                                '    BubbleEvent = False
                                '    Exit Sub
                                'Else
